...
::
::  USAGE
::    VirtRegTest.bat [OPTIONS]
::
::    /launcher:<path>   Path to VirtLauncher64.exe
::                       Default: look in %PATH% then same folder as this script
::    /vhive:<key>       Virtual store hive root
::                       Default: HKCU\VirtRegTest_Store_2026
::    /rbase:<key>       Real registry base for pre-seeded keys (HKCU only)
::                       Default: HKCU\Software\VirtRegTestReal_2026
::    /nocleanup         Skip cleanup of real and virtual registry seeds
::    /32                Use VirtLauncher32.exe instead of 64
...
::    Phase 2 – Run inner test battery INSIDE VirtLauncher; results logged
...
::    Phase 5 – Cleanup (unless /nocleanup)
...
:: ============================================================
:: DEFAULT CONFIGURATION (overridable via command-line)
:: ============================================================
set "LAUNCHER="
...
if "%~1"=="" goto :ARGDONE
set "ARG=%~1"
if /i "!ARG:~0,10!"=="/launcher:" ( set "LAUNCHER=!ARG:~10!"  & shift & goto :ARGLOOP )
if /i "!ARG:~0,7!"=="/vhive:"    ( set "VHIVE=!ARG:~7!"       & shift & goto :ARGLOOP )
if /i "!ARG:~0,7!"=="/rbase:"    ( set "RBASE=!ARG:~7!"       & shift & goto :ARGLOOP )
if /i "%~1"=="/nocleanup"        ( set "NOCLEANUP=1"           & shift & goto :ARGLOOP )
if /i "%~1"=="/32"               ( set "LAUNCHER_BITS=32"      & shift & goto :ARGLOOP )
if /i "%~1"=="/verbose"          ( set "VERBOSE=1"             & shift & goto :ARGLOOP )
if /i "%~1"=="--inside"          goto :RUN_INSIDE
shift & goto :ARGLOOP
:ARGDONE
...
:: Result files shared between outer and inner phases
set "INNER_LOG=%TEMP%\VirtRegTest_Inner.log"
set "INNER_SUMMARY=%TEMP%\VirtRegTest_InnerSummary.txt"
set "INNER_PASS_FILE=%TEMP%\VirtRegTest_InnerPass.txt"
set "INNER_FAIL_FILE=%TEMP%\VirtRegTest_InnerFail.txt"

:: ── Internal dispatch (called from within VirtLauncher) ──────────────────────
if defined VIRTLAUNCHER_REG goto :RUN_INSIDE
...
:: Empty result files
echo. > "%INNER_LOG%"
echo 0 > "%INNER_PASS_FILE%"
echo 0 > "%INNER_FAIL_FILE%"
...
if not exist "!LAUNCHER!" (
    ...
    echo                    Use /launcher:path\to\VirtLauncher64.exe
    exit /b 1
)
...
:: ============================================================
:: Phase 2: Run tests inside VirtLauncher
:: ============================================================
echo ================================================================
echo   PHASE 2 – Running inner test battery inside VirtLauncher
echo ================================================================
echo.

"!LAUNCHER!" -r "%VHIVE%" --exec cmd.exe /c ""%~f0" --inside"
set "LAUNCH_EC=!errorlevel!"

if !LAUNCH_EC! neq 0 (
    echo [LAUNCH] VirtLauncher exited with non-zero code: !LAUNCH_EC!
)
echo.
echo ----------------------------------------------------------------
echo   INNER-VIRT TEST OUTPUT
echo ----------------------------------------------------------------
if exist "%INNER_LOG%" ( type "%INNER_LOG%" ) else ( echo [ERROR] Inner log not found. )
echo ----------------------------------------------------------------
echo.

:: Read inner pass/fail counts
set /p "INNER_PASS=" < "%INNER_PASS_FILE%" 2>nul || set "INNER_PASS=0"
set /p "INNER_FAIL=" < "%INNER_FAIL_FILE%" 2>nul || set "INNER_FAIL=0"
...
:: ============================================================
if "%NOCLEANUP%"=="1" (
    echo [CLEANUP] Skipped (--nocleanup).
) else (
    echo ================================================================
    ...
    del /f /q "%INNER_LOG%" "%INNER_PASS_FILE%" "%INNER_FAIL_FILE%" >nul 2>&1
    echo.
)
...
:: Phase 6: Final Report
:: ============================================================
set /a "TOTAL_PASS=!INNER_PASS! + !ISO_PASS! + !VS_PASS!"
set /a "TOTAL_FAIL=!INNER_FAIL! + !ISO_FAIL! + !VS_FAIL!"
...
echo ================================================================
echo.
echo   Inner-virt tests  : PASS=!INNER_PASS!   FAIL=!INNER_FAIL!
...
:: ============================================================
:: ============================================================
:: INSIDE – All tests run under VirtLauncher
::          (every reg operation goes through VirtHook)
:: ============================================================
:: ============================================================
:RUN_INSIDE
...
call :LOG "  VirtStore (NT): %VIRTLAUNCHER_REG%"
call :LOG "================================================================"
call :LOG ""

:: Sanity: VIRTLAUNCHER_REG must be set
if not defined VIRTLAUNCHER_REG (
    call :FAIL "SANITY-1" "VIRTLAUNCHER_REG not set – VirtHook may not be active!"
    goto :INSIDE_DONE
)
call :PASS "SANITY-1" "VIRTLAUNCHER_REG is set: %VIRTLAUNCHER_REG%"

:: ── SECTION A: Basic Write / Read ────────────────────────────────────────────
call :SECT "A" "Basic Write / Read"

:: A-01: Create a brand-new HKCU key (no real equivalent)
reg add "%RBASE%\VirtCreatedKey" /v "VirtNewVal" /t REG_SZ /d "VIRT_NEW_DATA" /f >nul 2>&1
reg query "%RBASE%\VirtCreatedKey" /v "VirtNewVal" >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\SeedKey" /v "VirtAddedToSeed" /t REG_SZ /d "VIRT_ADDED" /f >nul 2>&1
reg query "%RBASE%\SeedKey" /v "VirtAddedToSeed" >nul 2>&1
if not errorlevel 1 (
...
reg query "%RBASE%\SeedKey" /v "RealDwordVal" 2>nul | findstr /i "REG_DWORD" >nul
if not errorlevel 1 (
...
reg add "HKLM\SOFTWARE\VirtRegTest_HKLM_2026" /v "HklmVirtVal" /t REG_SZ /d "HKLM_VIRT_DATA" /f >nul 2>&1
reg query "HKLM\SOFTWARE\VirtRegTest_HKLM_2026" /v "HklmVirtVal" >nul 2>&1
if not errorlevel 1 (
...
reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v "CurrentBuildNumber" >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\ShadowKey" /v "ShadowedVal" /t REG_SZ /d "VIRT_SHADOW_OVERRIDE" /f >nul 2>&1
reg query "%RBASE%\ShadowKey" /v "ShadowedVal" 2>nul | findstr /i "VIRT_SHADOW_OVERRIDE" >nul
if not errorlevel 1 (
...
reg query "%RBASE%\ShadowKey" /v "RealOnlyVal" 2>nul | findstr /i "REAL_ONLY_VALUE" >nul
if not errorlevel 1 (
...
reg query "%RBASE%\ShadowKey" /v "AnotherReal" 2>nul | findstr /i "ANOTHER_REAL_VALUE" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\ShadowKey" /v "VirtOnlyInShadow" /t REG_SZ /d "VIRT_ONLY" /f >nul 2>&1
set "B4_SH=0" & set "B4_RO=0" & set "B4_AR=0" & set "B4_VO=0"
for /f "delims=" %%L in ('reg query "%RBASE%\ShadowKey" 2^>nul') do (
...
reg add "%RBASE%\SeedKey" /v "RealDwordVal" /t REG_SZ /d "TYPE_CHANGED_TO_SZ" /f >nul 2>&1
reg query "%RBASE%\SeedKey" /v "RealDwordVal" 2>nul | findstr /i "TYPE_CHANGED_TO_SZ" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\MergeParent\VirtSubC" /f >nul 2>&1
set "C1_RA=0" & set "C1_RB=0" & set "C1_VC=0"
for /f "delims=" %%L in ('reg query "%RBASE%\MergeParent" 2^>nul') do (
...
reg query "%RBASE%\MergeParent\RealSubA" >nul 2>&1
if not errorlevel 1 ( call :PASS "C-02" "RealSubA directly accessible via full path" )
else                 ( call :FAIL "C-02" "RealSubA NOT accessible via full path" )

reg query "%RBASE%\MergeParent\RealSubB" >nul 2>&1
if not errorlevel 1 ( call :PASS "C-03" "RealSubB directly accessible via full path" )
else                 ( call :FAIL "C-03" "RealSubB NOT accessible via full path" )

:: C-04: Virtual subkey independently accessible
reg query "%RBASE%\MergeParent\VirtSubC" >nul 2>&1
if not errorlevel 1 ( call :PASS "C-04" "VirtSubC directly accessible via full path" )
else                 ( call :FAIL "C-04" "VirtSubC NOT accessible via full path" )

:: C-05: Add two more virt subkeys, no duplicates in full enum
reg add "%RBASE%\MergeParent\VirtSubD" /f >nul 2>&1
reg add "%RBASE%\MergeParent\VirtSubE" /f >nul 2>&1
...
reg add "%RBASE%\VirtOnlyDeleteMe" /v "v" /t REG_SZ /d "x" /f >nul 2>&1
reg delete "%RBASE%\VirtOnlyDeleteMe" /f >nul 2>&1
reg query "%RBASE%\VirtOnlyDeleteMe" >nul 2>&1
if errorlevel 1 (
...
reg add "%RBASE%\VirtCreatedKey" /v "TempVal" /t REG_SZ /d "ephemeral" /f >nul 2>&1
reg delete "%RBASE%\VirtCreatedKey" /v "TempVal" /f >nul 2>&1
reg query "%RBASE%\VirtCreatedKey" /v "TempVal" >nul 2>&1
if errorlevel 1 (
...
reg add "%RBASE%\SeedKey" /v "CoWDeleteTarget" /t REG_SZ /d "cow_delete" /f >nul 2>&1
reg delete "%RBASE%\SeedKey" /v "CoWDeleteTarget" /f >nul 2>&1
reg query "%RBASE%\SeedKey" /v "CoWDeleteTarget" >nul 2>&1
if errorlevel 1 (
...
reg add "%RBASE%\SeedKey" /v "RealStrVal_DeleteTest" /t REG_SZ /d "dummy_for_delete" /f >nul 2>&1
:: First ensure value exists in virtual (just wrote it above)
reg delete "%RBASE%\SeedKey" /v "RealStrVal_DeleteTest" /f >nul 2>&1
:: This was virt-only, so it should be gone:
reg query "%RBASE%\SeedKey" /v "RealStrVal_DeleteTest" >nul 2>&1
if errorlevel 1 (
...
reg delete "%RBASE%\SeedKey" /v "RealStrVal" /f >nul 2>&1
reg query "%RBASE%\SeedKey" /v "RealStrVal" >nul 2>&1
if errorlevel 1 (
...
reg delete "%RBASE%\DeleteTarget" /f >nul 2>&1
reg query "%RBASE%\DeleteTarget" >nul 2>&1
if errorlevel 1 (
...
reg add "%RBASE%\VirtSubtree\A" /v "va" /t REG_SZ /d "x" /f >nul 2>&1
reg add "%RBASE%\VirtSubtree\B" /v "vb" /t REG_SZ /d "x" /f >nul 2>&1
reg add "%RBASE%\VirtSubtree\A\Child" /v "vc" /t REG_SZ /d "x" /f >nul 2>&1
reg delete "%RBASE%\VirtSubtree" /f >nul 2>&1
reg query "%RBASE%\VirtSubtree" >nul 2>&1
if errorlevel 1 (
...
reg query "%RBASE%\VirtSubtree\A" >nul 2>&1
if errorlevel 1 (
...
reg add "%RBASE%\MergeParent\VirtSubC" /f >nul 2>&1
reg delete "%RBASE%\MergeParent\VirtSubC" /f >nul 2>&1
reg query "%RBASE%\MergeParent\RealSubA" >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\N1\N2\N3\N4\N5" /v "VirtDeepNew" /t REG_SZ /d "DEEP_VIRT_VALUE" /f >nul 2>&1
reg query "%RBASE%\N1\N2\N3\N4\N5" /v "VirtDeepNew" 2>nul | findstr /i "DEEP_VIRT_VALUE" >nul
if not errorlevel 1 (
...
reg query "%RBASE%\Deep\L1\L2\L3\L4\L5" /v "DeepRealVal" 2>nul | findstr /i "DEEP_REAL" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\Deep\L1\L2\L3\L4\L5" /v "VirtDeepCoW" /t REG_SZ /d "COW_AT_DEPTH_5" /f >nul 2>&1
reg query "%RBASE%\Deep\L1\L2\L3\L4\L5" /v "VirtDeepCoW" 2>nul | findstr /i "COW_AT_DEPTH_5" >nul
if not errorlevel 1 (
...
reg add "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\A\B\C" /v "HklmDeepVal" /t REG_SZ /d "HKLM_DEEP" /f >nul 2>&1
reg query "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\A\B\C" /v "HklmDeepVal" 2>nul | findstr /i "HKLM_DEEP" >nul
if not errorlevel 1 (
...
:: F-01: REG_SZ
reg add "%FTKEY%" /v "SzVal" /t REG_SZ /d "hello world string" /f >nul 2>&1
reg query "%FTKEY%" /v "SzVal" 2>nul | findstr /i "REG_SZ" >nul
if not errorlevel 1 ( call :PASS "F-01" "REG_SZ: written and type preserved on read" )
else                 ( call :FAIL "F-01" "REG_SZ: type wrong or not readable" )
reg query "%FTKEY%" /v "SzVal" 2>nul | findstr /i "hello world string" >nul
if not errorlevel 1 ( call :PASS "F-01b" "REG_SZ: data value correct on read" )
else                 ( call :FAIL "F-01b" "REG_SZ: data value wrong on read" )

:: F-02: REG_EXPAND_SZ
reg add "%FTKEY%" /v "ExpSzVal" /t REG_EXPAND_SZ /d "%%TEMP%%\virt_test.tmp" /f >nul 2>&1
reg query "%FTKEY%" /v "ExpSzVal" 2>nul | findstr /i "REG_EXPAND_SZ" >nul
if not errorlevel 1 ( call :PASS "F-02" "REG_EXPAND_SZ: type preserved on read" )
else                 ( call :FAIL "F-02" "REG_EXPAND_SZ: type wrong or not readable" )

:: F-03: REG_DWORD
reg add "%FTKEY%" /v "DwordVal" /t REG_DWORD /d 0xDEAD1234 /f >nul 2>&1
reg query "%FTKEY%" /v "DwordVal" 2>nul | findstr /i "REG_DWORD" >nul
if not errorlevel 1 ( call :PASS "F-03" "REG_DWORD: type preserved on read" )
else                 ( call :FAIL "F-03" "REG_DWORD: type wrong or not readable" )
reg query "%FTKEY%" /v "DwordVal" 2>nul | findstr /i "0xdead1234" >nul
if not errorlevel 1 ( call :PASS "F-03b" "REG_DWORD: value 0xDEAD1234 correct" )
else                 ( call :FAIL "F-03b" "REG_DWORD: value 0xDEAD1234 wrong on read" )

:: F-04: REG_QWORD
reg add "%FTKEY%" /v "QwordVal" /t REG_QWORD /d 0x0102030405060708 /f >nul 2>&1
reg query "%FTKEY%" /v "QwordVal" 2>nul | findstr /i "REG_QWORD" >nul
if not errorlevel 1 ( call :PASS "F-04" "REG_QWORD: type preserved on read" )
else                 ( call :FAIL "F-04" "REG_QWORD: type wrong (may need Win10+)" )

:: F-05: REG_BINARY
reg add "%FTKEY%" /v "BinVal" /t REG_BINARY /d "DEADBEEF01020304CAFEBABE" /f >nul 2>&1
reg query "%FTKEY%" /v "BinVal" 2>nul | findstr /i "REG_BINARY" >nul
if not errorlevel 1 ( call :PASS "F-05" "REG_BINARY: type preserved on read" )
else                 ( call :FAIL "F-05" "REG_BINARY: type wrong or not readable" )
reg query "%FTKEY%" /v "BinVal" 2>nul | findstr /i "de ad be ef" >nul
if not errorlevel 1 ( call :PASS "F-05b" "REG_BINARY: data bytes correct on read" )
else                 ( call :FAIL "F-05b" "REG_BINARY: data bytes wrong or not matching" )

:: F-06: REG_MULTI_SZ
reg add "%FTKEY%" /v "MultiSzVal" /t REG_MULTI_SZ /d "FirstLine\0SecondLine\0ThirdLine" /f >nul 2>&1
reg query "%FTKEY%" /v "MultiSzVal" 2>nul | findstr /i "REG_MULTI_SZ" >nul
if not errorlevel 1 ( call :PASS "F-06" "REG_MULTI_SZ: type preserved on read" )
else                 ( call :FAIL "F-06" "REG_MULTI_SZ: type wrong or not readable" )

:: F-07: REG_NONE
reg add "%FTKEY%" /v "NoneVal" /t REG_NONE /d "" /f >nul 2>&1
reg query "%FTKEY%" /v "NoneVal" >nul 2>&1
if not errorlevel 1 ( call :PASS "F-07" "REG_NONE: written and readable" )
else                 ( call :FAIL "F-07" "REG_NONE: not readable" )
...
for %%N in (SzVal ExpSzVal DwordVal QwordVal BinVal MultiSzVal NoneVal) do (
    reg query "%FTKEY%" /v "%%N" >nul 2>&1 && set /a "F8COUNT+=1"
)
if !F8COUNT!==7 (
...
reg add "%RBASE%\DefaultValKey" /ve /t REG_SZ /d "default_data" /f >nul 2>&1
reg query "%RBASE%\DefaultValKey" /ve 2>nul | findstr /i "default_data" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\DefaultValKey" /ve /t REG_SZ /d "default_override" /f >nul 2>&1
reg query "%RBASE%\DefaultValKey" /ve 2>nul | findstr /i "default_override" >nul
if not errorlevel 1 (
...
reg query "%RBASE%\DefaultValKey" 2>nul | findstr /i "(Default)" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\DefaultValKey" /v "NamedSibling" /t REG_SZ /d "named" /f >nul 2>&1
reg query "%RBASE%\DefaultValKey" /ve >nul 2>&1
reg query "%RBASE%\DefaultValKey" /v "NamedSibling" >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\LifecycleKey" /v "Val1" /t REG_SZ /d "lifecycle_one"  /f >nul 2>&1
reg add "%RBASE%\LifecycleKey" /v "Val2" /t REG_SZ /d "lifecycle_two"  /f >nul 2>&1
reg add "%RBASE%\LifecycleKey" /v "Val3" /t REG_DWORD /d 0x12345678    /f >nul 2>&1
reg query "%RBASE%\LifecycleKey" /v "Val1" 2>nul | findstr /i "lifecycle_one" >nul
if not errorlevel 1 ( call :PASS "H-01" "Lifecycle: Val1 readable after close+reopen" )
else                 ( call :FAIL "H-01" "Lifecycle: Val1 NOT readable after close+reopen" )
reg query "%RBASE%\LifecycleKey" /v "Val2" 2>nul | findstr /i "lifecycle_two" >nul
if not errorlevel 1 ( call :PASS "H-02" "Lifecycle: Val2 readable after close+reopen" )
else                 ( call :FAIL "H-02" "Lifecycle: Val2 NOT readable after close+reopen" )

:: H-03: Overwrite a value, close, reopen – updated value must persist
reg add "%RBASE%\LifecycleKey" /v "Val1" /t REG_SZ /d "lifecycle_one_UPDATED" /f >nul 2>&1
reg query "%RBASE%\LifecycleKey" /v "Val1" 2>nul | findstr /i "lifecycle_one_UPDATED" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\LifecycleKey" /v "TwoCycle" /t REG_SZ /d "cycle1" /f >nul 2>&1
reg add "%RBASE%\LifecycleKey" /v "TwoCycle" /t REG_SZ /d "cycle2" /f >nul 2>&1
reg query "%RBASE%\LifecycleKey" /v "TwoCycle" 2>nul | findstr /i "cycle2" >nul
if not errorlevel 1 (
...
reg add "%RBASE%\DispNewKey" /f >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\SeedKey" /f >nul 2>&1
if not errorlevel 1 (
...
reg query "%RBASE%\SeedKey" /v "RealDwordVal" >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\Key With Spaces In Name" /v "SpaceVal" /t REG_SZ /d "space_key_ok" /f >nul 2>&1
reg query "%RBASE%\Key With Spaces In Name" /v "SpaceVal" 2>nul | findstr /i "space_key_ok" >nul
if not errorlevel 1 ( call :PASS "J-01" "Key name with spaces: write + read OK" )
else                 ( call :FAIL "J-01" "Key name with spaces: FAILED" )

:: J-02: Value name with spaces
reg add "%RBASE%\SeedKey" /v "Value Name With Spaces" /t REG_SZ /d "val_spaces_ok" /f >nul 2>&1
reg query "%RBASE%\SeedKey" /v "Value Name With Spaces" 2>nul | findstr /i "val_spaces_ok" >nul
if not errorlevel 1 ( call :PASS "J-02" "Value name with spaces: write + read OK" )
else                 ( call :FAIL "J-02" "Value name with spaces: FAILED" )

:: J-03: Key with numbers and underscores
reg add "%RBASE%\Key_123_Test456" /v "v" /t REG_SZ /d "numkey_ok" /f >nul 2>&1
reg query "%RBASE%\Key_123_Test456" /v "v" >nul 2>&1
if not errorlevel 1 ( call :PASS "J-03" "Key name with numbers+underscores: OK" )
else                 ( call :FAIL "J-03" "Key name with numbers+underscores: FAILED" )
...
reg add "%RBASE%\%LONGNAME%" /v "lv" /t REG_SZ /d "long_name_ok" /f >nul 2>&1
reg query "%RBASE%\%LONGNAME%" /v "lv" >nul 2>&1
if not errorlevel 1 ( call :PASS "J-04" "Long key name (72 chars): write + read OK" )
else                 ( call :FAIL "J-04" "Long key name (72 chars): FAILED" )
...
reg add "%RBASE%\LargeDataKey" /v "BigSzVal" /t REG_SZ /d "!BIGSTR!" /f >nul 2>&1
reg query "%RBASE%\LargeDataKey" /v "BigSzVal" 2>nul | findstr /i "REG_SZ" >nul
if not errorlevel 1 ( call :PASS "J-05" "Large REG_SZ value (~370 chars): type readable" )
else                 ( call :FAIL "J-05" "Large REG_SZ value: FAILED" )
...
reg add "%RBASE%\LargeDataKey" /v "BigBinVal" /t REG_BINARY /d "!BIGBIN!" /f >nul 2>&1
reg query "%RBASE%\LargeDataKey" /v "BigBinVal" 2>nul | findstr /i "REG_BINARY" >nul
if not errorlevel 1 ( call :PASS "J-06" "Large REG_BINARY (128 bytes): type readable" )
else                 ( call :FAIL "J-06" "Large REG_BINARY: FAILED" )
...
:: K-01: ShadowKey should have: ShadowedVal(virt), RealOnlyVal(real), AnotherReal(real), VirtOnlyInShadow(virt) = 4 values
::        NtQueryKey value count from virtual handle = 2 (ShadowedVal, VirtOnlyInShadow)
::        We test what PowerShell reports (uses RegQueryInfoKey which calls NtQueryKey)
...
reg add "%RBASE%\ExportTest\Sub1" /v "ev1" /t REG_SZ    /d "export_val_one" /f >nul 2>&1
reg add "%RBASE%\ExportTest\Sub2" /v "ev2" /t REG_DWORD /d 0x42             /f >nul 2>&1
reg add "%RBASE%\ExportTest"      /v "ev3" /t REG_SZ    /d "export_root_val" /f >nul 2>&1
reg export "%RBASE%\ExportTest" "%TEMP%\VRTExport.reg" /y >nul 2>&1
...
set "L2FOUND=0"
for /f "delims=" %%L in ('reg query "%RBASE%\ExportTest" /s 2^>nul') do (
...
reg delete "%RBASE%\ExportTest" /f >nul 2>&1
reg query "%RBASE%\ExportTest" >nul 2>&1
if errorlevel 1 (
...
reg query "%RBASE%\ExportTest\Sub1" >nul 2>&1
if errorlevel 1 (
...
reg add "%RBASE%\CrossHive" /v "HkcuSideVal" /t REG_SZ /d "hkcu_side" /f >nul 2>&1
reg add "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\CrossHive" /v "HklmSideVal" /t REG_SZ /d "hklm_side" /f >nul 2>&1
reg query "%RBASE%\CrossHive" /v "HkcuSideVal" 2>nul | findstr /i "hkcu_side" >nul
if not errorlevel 1 ( call :PASS "M-01a" "HKCU\CrossHive\HkcuSideVal readable" )
else                 ( call :FAIL "M-01a" "HKCU\CrossHive\HkcuSideVal NOT readable" )
reg query "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\CrossHive" /v "HklmSideVal" 2>nul | findstr /i "hklm_side" >nul
if not errorlevel 1 ( call :PASS "M-01b" "HKLM\CrossHive\HklmSideVal readable" )
else                 ( call :FAIL "M-01b" "HKLM\CrossHive\HklmSideVal NOT readable" )

:: M-02: HKCU path must NOT see the HKLM value
reg query "%RBASE%\CrossHive" /v "HklmSideVal" >nul 2>&1
if errorlevel 1 (
...
reg query "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\CrossHive" /v "HkcuSideVal" >nul 2>&1
if errorlevel 1 (
...
reg add "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\DeleteMe" /v "hklm_del" /t REG_SZ /d "x" /f >nul 2>&1
reg delete "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\DeleteMe" /f >nul 2>&1
reg query "HKLM\SOFTWARE\VirtRegTest_HKLM_2026\DeleteMe" >nul 2>&1
if errorlevel 1 (
...
reg add "HKCR\VirtRegTest_HKCR_2026" /v "HkcrVal" /t REG_SZ /d "hkcr_virt_data" /f >nul 2>&1
reg query "HKCR\VirtRegTest_HKCR_2026" /v "HkcrVal" >nul 2>&1
if not errorlevel 1 (
...
reg query "HKCR\VirtRegTest_HKCR_2026" >nul 2>&1
if not errorlevel 1 (
...
reg add "%RBASE%\RenameSource" /v "rv" /t REG_SZ /d "rename_data" /f >nul 2>&1
powershell -NoProfile -Command ^
    "try {" ^
...
if not errorlevel 1 (
    ...
    reg query "%RBASE%\RenameTarget" >nul 2>&1
    if not errorlevel 1 ( call :PASS "O-02" "RenameTarget key accessible after rename" )
    else                 ( call :FAIL "O-02" "RenameTarget NOT accessible after rename" )
    reg query "%RBASE%\RenameSource" >nul 2>&1
    if errorlevel 1 ( call :PASS "O-03" "RenameSource gone after rename" )
    else            ( call :FAIL "O-03" "RenameSource STILL PRESENT after rename!" )
) else (
...
for /L %%I in (1,1,20) do (
    reg add "%RBASE%\StressParent\Sub%%I" /v "v" /t REG_SZ /d "stress_%%I" /f >nul 2>&1
...
set "Q2COUNT=0"
for /f "delims=" %%L in ('reg query "%RBASE%\StressParent" 2^>nul') do (
...
for /L %%I in (1,1,25) do (
    reg add "%RBASE%\StressManyVals" /v "Val%%I" /t REG_SZ /d "stress_val_%%I" /f >nul 2>&1
...
reg query "%RBASE%\StressManyVals" /v "Val7"  2>nul | findstr /i "stress_val_7"  >nul && set "Q4A=1" || set "Q4A=0"
reg query "%RBASE%\StressManyVals" /v "Val15" 2>nul | findstr /i "stress_val_15" >nul && set "Q4B=1" || set "Q4B=0"
reg query "%RBASE%\StressManyVals" /v "Val25" 2>nul | findstr /i "stress_val_25" >nul && set "Q4C=1" || set "Q4C=0"
...
:: Save counts for outer phase to read
echo %PASS% > "%INNER_PASS_FILE%"
echo %FAIL% > "%INNER_FAIL_FILE%"
